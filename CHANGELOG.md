...
- Cart adds record which door the part came through (`via`: photo, search, for_you,
  ...
  chip, the share that found nothing and the queries that did, the share that led to a cart
  add); a fifth of searches finding nothing raises an issue naming what was typed.
...
- Search facets: when a query matches several variants of one name, `GET /search/facets`
  ...
  A full page of ten search results offers "More results", which appends the next page in
  the same ordering. Fixes a race
