...
| marketplace | customer model from orders (profiles, segments as an industry proxy, complements); personalised search and photo ranking as a tie-breaker; For-you strip with re-order prompts; `mcv simulate-market` measures the lift, with and without the shop id on the same lookups, on hundreds of shops |
