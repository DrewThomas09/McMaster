...
Everything except the real imagery is built and tested (290+ tests, including
...
| sizing | Measure tool and coin hint (lands on the coin 99.7% of the time on the demo catalog, two false hints in 400 coinless photos); nominal pipe size -> real OD/ID; nut width per thread size; thread pitch from the photo (NPT vs BSP, coarse vs fine); catalog-page importer. A quarter in the frame: photo top-1 80% -> 86%, 76% -> 84% on parts a shop never bought (synthetic marketplace, 2026-09-09) |
| marketplace | customer model from orders (profiles, segments as an industry proxy, complements); personalised search and photo ranking as a tie-breaker; For-you strip with re-order prompts; facet chips when a name has variants (one tap narrows; a spec typed verbatim leads); `mcv simulate-market` measures the lift with and without the shop id on the same lookups: 1000 shops, 15k orders, search top-1 49% -> 66%, photo 82% -> 85%, industries recovered with purity 0.90 (2026-09-09); on an 800-part catalog the chips lift never-bought search top-1 22% -> 25% for a customer who knows the size (2026-09-12) |
